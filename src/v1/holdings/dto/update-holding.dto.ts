...
export const UpdateHoldingSchema = z.object({
  ...
  units: z.string().regex(/^\d+(\.\d+)?$/, 'Units must be a valid decimal number').nullable().optional(),
  avg_buy_price: z.string().regex(/^\d+(\.\d+)?$/, 'Average buy price must be a valid decimal number').nullable().optional(),
  current_price: z.string().regex(/^\d+(\.\d+)?$/, 'Current price must be a valid decimal number').nullable().optional(),
